...
internal static class WorkflowConsoleRenderer
{
  ...
  internal static void PrintInputImage(ChatMessage message)
  {
    ...
    try
    {
      ...
      {
        fs.Write(imageContent.Data.Span);
      }
  ...
  internal static void PrintTimingSummary(Dictionary<string, TimeSpan> executorDurations, TimeSpan totalTime, int tokenCount)
  {
    ...
    var summaryLines = tokenCount > 0
      ? $"[yellow]⏱ Total Workflow Time:[/] [white]{totalTime.TotalSeconds:F2}s[/]{Environment.NewLine}[yellow]📊 Streamed Tokens:[/]    [white]~{tokenCount}[/]"
      : $"[yellow]⏱ Total Workflow Time:[/] [white]{totalTime.TotalSeconds:F2}s[/]";
  ...
  internal static void UpdateDashboardPanels(Layout root, DashboardState state, string cachedColoredText, string status)
  {
    ...
    // ── Output panel (always updated — streaming tokens arrive every refresh) ──
    ...
    // ── Events panel (only when events changed) ──
    ...
    // ── Tools panel (only when tools changed) ──
    if (state.ToolsDirty)
    {
    ...
    // ── Metrics panel (always updated — elapsed time changes every refresh) ──
    ...
    metricsBuilder.AppendLine($"Status: {status}");
    metricsBuilder.AppendLine($"Elapsed: {state.Elapsed.Elapsed.TotalSeconds:F1}s");
    metricsBuilder.AppendLine($"Active: {state.ActiveAgent ?? "-"}");
    metricsBuilder.AppendLine($"Agents: {state.UniqueAgents.Count}");
    metricsBuilder.AppendLine($"Tokens: ~{state.TokenCount}  |  {tokPerSec:F1} tok/s");

    if (state.ExecutorDurations.Count > 0)
    {
      metricsBuilder.AppendLine("Executor (duration / invocations / tokens):");
      foreach (var kvp in state.ExecutorDurations.OrderByDescending(kvp => kvp.Value))
      {
        ...
        metricsBuilder.AppendLine($"  {shortId}: {kvp.Value.TotalSeconds:F2}s / {executorInvocations} / {tokens}");
      }
    }

    if (state.SuperStepDurations.Count > 0)
    {
      metricsBuilder.Append("Steps:");
      foreach (var kvp in state.SuperStepDurations.OrderBy(kvp => kvp.Key))
      {
        metricsBuilder.Append($" #{kvp.Key}({kvp.Value.TotalSeconds:F1}s)");
      }
    }

    root["Metrics"].Update(BuildDashboardPanel("📊 Metrics", metricsBuilder.ToString(), Color.Gold1));

    // ── Review panel (only when review changed) ──
    ...
    Dictionary<string, string>? agentColorMap)
  {
    foreach (var (agentKey, buffer) in perAgentOutput)
    {
      ...
      var coloredBuilder = new StringBuilder(escapedText.Length + 64);
      SpectreDashboardSession.AppendColoredMarkup(coloredBuilder, escapedText, color);
      var content = coloredBuilder.ToString();
