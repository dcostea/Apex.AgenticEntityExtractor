...
builder.Services.AddSingleton<Workflow>(sp =>
{
  ...
  .BuildSequential("SequentialEntityExtractionWorkflow", 
    [entityAgent, relationshipAgent, summaryAgent]);
});

builder.AddDevUI();
builder.AddWorkflow("SequentialEntityExtractionWorkflow", (sp, name) =>
{
...
{
  AIAgent workflowAgent = workflow.AsAIAgent("SequentialEntityExtractionWorkflow");
