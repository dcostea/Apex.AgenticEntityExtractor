      ...
      await foreach (var evt in run.WatchStreamAsync())
      {
        ...
        switch (evt)
        {
          case SuperStepStartedEvent stepStarted:
            ...
            {
              state.LogEvent($"⏳ Step #{stepStarted.StepNumber} ← Senders: {string.Join(", ", startInfo.SendingExecutors.Select(GetShortExecutorId))}");
            }
            else
            {
              state.LogEvent($"⏳ Step #{stepStarted.StepNumber}");
            }
          ...
          case SuperStepCompletedEvent stepCompleted:
            ...
            {
              string activatedSuffix = completionInfo.ActivatedExecutors.Count != 0
                ? $" → Activated: {string.Join(", ", completionInfo.ActivatedExecutors.Select(GetShortExecutorId))}"
                : "";
              if (completionInfo.InstantiatedExecutors.Count != 0)
              {
                state.LogEvent($"✅ Step #{stepCompleted.StepNumber}{activatedSuffix}");
            ...
            {
              state.LogEvent($"✅ Step #{stepCompleted.StepNumber}");
            }
            break;
