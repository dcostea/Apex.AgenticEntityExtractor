...
builder.AddAIAgent("ReporterAgent", (sp, _) =>
{
  var agentBuilder = sp.GetRequiredService<IExtractorAgentsBuilder>();
  var configuration = sp.GetRequiredService<IConfiguration>();
  return agentBuilder.BuildReporterAgent(configuration.GetValue<ChatProvider>("Provider"));
});
builder.AddAIAgent("AnalystAgent", (sp, _) =>
{
  var agentBuilder = sp.GetRequiredService<IExtractorAgentsBuilder>();
  var configuration = sp.GetRequiredService<IConfiguration>();
  return agentBuilder.BuildAnalystAgent(configuration.GetValue<ChatProvider>("Provider"));
});
